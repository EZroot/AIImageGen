...
# video_pipe_model_id = "stabilityai/stable-video-diffusion-img2vid-xt-1-1"  sucks? (might be my setup tho)
video_pipe_model_id = "stabilityai/stable-video-diffusion-img2vid"  # great!
# video_pipe_model_id = "Wan-AI/Wan2.1-I2V-14B-480P"

logging.info("Loading StableVideoDiffusionPipeline model...")
controlnet = ControlNetModel.from_pretrained("lllyasviel/sd-controlnet-openpose", torch_dtype=torch.float16)
video_pipe = StableDiffusionControlNetPipeline.from_pretrained(
    video_pipe_model_id,
...
video_pipe.unet.set_attn_processor(CrossFrameAttnProcessor(batch_size=2))
video_pipe.controlnet.set_attn_processor(CrossFrameAttnProcessor(batch_size=2))
...
def generate():
    ...
    The GIF is generated using the StableVideoDiffusionPipeline and saved losslessly,
    with the second half of the GIF being a reversed copy of the first half.
    """
    ...
    gen_width = 1920
    gen_height = 1080
    video_fps = data.get("video_fps", 2)
    ...
    target_decode_chunk_size = 8 # 16 # or use 8
    target_width, target_height = 1024, 576 # use this with stable diffusion img2vid
    ...
    try:
        latents = torch.randn((1, 4, 64, 64), device=device, dtype=torch.float16).repeat(num_frames, 1, 1, 1)
        video_output = video_pipe(
            image=video_image,
            decode_chunk_size=target_decode_chunk_size,
            generator=generator,
            latents=latents,
    ...
    # Export the video frames as a lossless GIF file with a reversed sequence appended.
    gif_filename = f"{uuid.uuid4().hex}.gif"
    gif_path = os.path.join(app.config['GIF_SAVE_DIRECTORY'], gif_filename)
    try:
        # Assuming video_output.frames is a list containing one sequence of frames
        frames = video_output.frames[0]
        # Convert each frame to a numpy array (RGB) for imageio
        frames_np = [np.array(frame.convert("RGB")) for frame in frames]
        # Create a reversed copy of frames, omitting the last frame to avoid duplication
        reversed_frames = frames_np[:-1][::-1]
        all_frames = frames_np + reversed_frames
        # Set loop=0 to ensure the GIF loops forever
        imageio.mimsave(gif_path, all_frames, fps=video_fps, loop=0)
    except Exception as e:
        logging.error(f"Error exporting GIF: {e}")
        return jsonify({"error": str(e)}), 500
